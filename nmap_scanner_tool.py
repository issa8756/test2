...
def scan_ips(ips, scan_type, save_to_file, speed_option, ports):
    ...
    # Convert the list of ports into a string with the `-p` flag
    if ports:
        ports_str = f"-p{','.join(ports)}"  # Add `-p` flag before the list of ports
    else:
        ports_str = ""
    ...
    for host in nm.all_hosts():
        ...
        for proto in nm[host].all_protocols():
            result += f"\nProtocol: {proto}"
            for port in nm[host][proto].keys():  # Get all ports for the protocol
...
def main():
    ...
    # Convert IPs and ports arguments to lists
    ips_list = args.ips.split(",")
    ports_list = args.ports.split(",") if args.ports else []

    # Run the scan
    scan_ips(ips_list, args.type, args.output, args.speed, ports_list)
